...
<task type="auto">
  ...
  <action>
    ...
    3. Check if STATUS_COLORS covers these statuses. If not, keep a local mapping OR extend statusColors.ts
    4. If statuses differ: Create a local staffStatusColors that maps to existing tokens:
       - active -> confirmed (sage)
       - inactive -> draft (charcoal)
       - on-leave -> pending (lavender) or a custom pattern
    5. Delete the original local definition and use the mapping
