...
<task type="auto">
  <name>Task 1: Apply ownerPortalOnly middleware to owner routes</name>
  <files>apps/api/src/app.ts</files>
  <action>
First, examine app.ts and routes/index.ts to understand the route structure. Then apply the ownerPortalOnly middleware to protect owner routes from staff token access.

**Routes that should be owner-only (apply ownerPortalOnly):**
...
**Routes that staff CAN access (do NOT apply ownerPortalOnly):**
...
**Strategy:** Apply middleware at the router mount level for efficiency.
...
<task type="auto">
  <name>Task 2: Update auth.ts JWTPayload interface for portalType</name>
