...
must_haves:
  truths:
    - "Owner can send initial email invite to staff member (POST /invite - already exists)"
    - "Staff can set password via magic link (/staff/setup page - already exists)"
    - "Owner can manually resend invite to staff member (POST /invite/resend/:staffId)"
  ...
  artifacts:
    - path: "apps/api/src/routes/staffPortal.ts"
      provides: "POST /invite endpoint (existing) and POST /invite/resend/:staffId endpoint (new)"
      contains: "resend"
...
Note: AUTH-01 (initial invite sending) is already implemented via POST /api/v1/staff-portal/invite (lines 307-411 of staffPortal.ts). AUTH-02 (staff can set password via magic link) is also already implemented - the /staff/setup page exists at apps/web/src/app/staff/setup/page.tsx and calls POST /api/v1/staff-portal/setup (lines 417-480 of staffPortal.ts). This plan adds the resend capability and invite status tracking.
</objective>
