...
must_haves:
  truths:
    ...
    - "Dashboard appointments filter to staff's assigned locations (or all if no location assignments)"
    - "Dashboard conditionally shows client phone when staffCanViewClientContact is true"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    3. Modify the todayAppointments query to filter by locationId (only if staff has location assignments).
       **IMPORTANT:** If staffLocationIds.length === 0, do NOT apply location filter - staff sees all salon appointments.
    ```typescript
    const todayAppointments = await prisma.appointment.findMany({
      ...
      select: {
        ...
        endTime: true,
        status: true,
        notes: true,  // Include notes for detail modal
...
- Dashboard API includes notes in appointment select for detail modal
- Dashboard API includes price in service select for detail modal
- Appointments are filtered to staff's assigned locations (no filter if staff has zero location assignments)
- Dashboard conditionally shows client phone when visibility setting allows
