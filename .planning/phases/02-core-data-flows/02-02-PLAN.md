...
must_haves:
  ...
  key_links:
    ...
    - from: "apps/web/src/components/LocationSwitcher.tsx"
      to: "setSelectedLocationId"
      via: "context setter call"
      pattern: "setSelectedLocationId"
---
...
Verify LocationSwitcher component wiring:
- Open LocationSwitcher.tsx and confirm it imports useLocationContext
- Confirm it calls setSelectedLocationId on selection (search for setSelectedLocationId call)
- If wiring is missing, add the proper context integration
...
- Grep LocationSwitcher.tsx: confirm pattern `useLocationContext` is imported and `setSelectedLocationId` is called
  </verify>
  <done>Location switching works and persists correctly with proper context wiring</done>
</task>
...
<task type="auto">
  <name>Task 4: Verify location hours restrict booking widget availability</name>
  <files>apps/web/src/app/embed/[slug]/page.tsx, apps/api/src/routes/appointments.ts</files>
  <action>
Test that location hours affect booking widget time slots:
1. Set Location A hours: Monday 9:00 AM - 5:00 PM
2. Set Location B hours: Monday 10:00 AM - 8:00 PM
3. Open the booking widget for Location A

Verify booking widget respects location hours:
- Time slots before 9:00 AM are NOT available at Location A
- Time slots after 5:00 PM are NOT available at Location A
- Switch to Location B in booking widget
- Time slots should now extend to 8:00 PM

Test closed day behavior:
1. Set Sunday as closed for Location A
2. Open booking widget for Location A
3. Select a Sunday date

Verify:
- No time slots are available for Sunday
- Message indicates location is closed

API verification:
- GET /api/v1/booking/:slug/availability?date=YYYY-MM-DD&locationId=X
- Response should only include slots within location hours
- Slots outside location hours should NOT appear

Fix any issues found:
- If booking widget shows all time slots regardless of hours, check availability endpoint filters by location hours
- If closed days still show slots, ensure isClosed check in availability logic
  </action>
  <verify>
With Location A hours set to 9-5 on Monday:
- Open booking widget, select Monday
- Verify earliest slot is 9:00 AM or later
- Verify latest slot ends by 5:00 PM
- curl GET /api/v1/booking/:slug/availability confirms slots respect hours
  </verify>
  <done>Location hours correctly restrict booking widget time slot availability</done>
</task>
...
6. Open booking widget and verify time slots respect location hours
7. Delete a non-primary location (or the one created)
...
- Booking widget shows only time slots within location hours
