...
must_haves:
  truths:
    ...
    - "Calendar shows only staff assigned to selected location"
  artifacts:
  ...
  key_links:
    ...
    - from: "apps/web/src/app/calendar/page.tsx"
      to: "staff dropdown filter"
      via: "useLocationContext for filtering"
...
Calendar UI verification (mandatory):
1. Navigate to calendar page in browser
2. Use LocationSwitcher to select Location 1
3. Open the staff filter/dropdown on the calendar
4. Visually confirm dropdown contains ONLY:
   - Staff members assigned to Location 1
   - Staff members with NO location assignments (unassigned)
5. Staff assigned ONLY to Location 2 must NOT appear in dropdown
  </verify>
  <done>Staff scheduling respects location assignments in both API and calendar UI</done>
</task>
...
4. Open calendar page, select Location 1, verify staff dropdown shows only Location 1 staff + unassigned
5. Create an appointment at specific location
6. Verify appointment-staff-location relationships are correct
...
- Calendar/scheduling shows only relevant staff per location (verified in UI)
- Calendar staff dropdown filters correctly based on selected location
