...
```
  ...
  <verify>
1. Run `cd apps/api && pnpm build` to verify TypeScript compilation.
2. Start the API server and verify timezone field appears in actual responses:
   - `curl -s -H "Authorization: Bearer $TOKEN" http://localhost:3001/api/dashboard/stats | grep -q '"timezone"' && echo "stats: PASS" || echo "stats: FAIL"`
   - `curl -s -H "Authorization: Bearer $TOKEN" http://localhost:3001/api/dashboard/today | grep -q '"timezone"' && echo "today: PASS" || echo "today: FAIL"`
   - `curl -s -H "Authorization: Bearer $TOKEN" http://localhost:3001/api/dashboard/recent-activity | grep -q '"timezone"' && echo "recent-activity: PASS" || echo "recent-activity: FAIL"`

   All three endpoints must return timezone field in response. If no auth token available, inspect response JSON manually in browser devtools Network tab.
  </verify>
  <done>Dashboard API endpoints include salon timezone in responses, verified by runtime API check showing timezone field in /stats, /today, and /recent-activity responses.</done>
</task>
...
<task type="auto">
  ...
  <action>
Update useDashboard (already refactored by 07-03) to extract and expose the salon timezone:

IMPORTANT: Plan 07-03 has already refactored useDashboard to use independent queries. This task adds timezone support to that existing structure.

1. Update ApiStatsResponse interface to include timezone:
...
2. Update DashboardStats interface to include timezone:
...
3. In fetchStats function (from 07-03), add timezone to the return object:

```typescript
// In the return statement of fetchStats, add:
timezone: apiData?.timezone ?? 'UTC',
```

4. In the useDashboard return statement, add timezone:

```typescript
return {
  // ... existing returns from 07-03 ...
