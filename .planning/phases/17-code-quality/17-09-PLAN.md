...
<task type="auto">
  ...
  <files>apps/api/src/routes/*.ts, apps/api/src/services/*.ts, apps/api/src/middleware/*.ts</files>
  <action>
Before enabling noImplicitAny, verify all source files are properly typed:

1. Check for remaining `: any` patterns in routes:
```bash
cd apps/api && grep -r ": any" src/routes/ || echo "No any types found in routes"
```

2. Check for remaining `: any` patterns in services:
```bash
cd apps/api && grep -r ": any" src/services/ || echo "No any types found in services"
```

3. Check for remaining `: any` patterns in middleware:
```bash
cd apps/api && grep -r ": any" src/middleware/ || echo "No any types found in middleware"
```

4. Check that all route files import Prisma types:
```bash
cd apps/api && grep -L "import.*Prisma" src/routes/*.ts || echo "All files import Prisma"
```

5. Run TypeScript check to see current error count:
```bash
cd apps/api && npx tsc --noEmit 2>&1 | wc -l
```

If any files in routes/, services/, or middleware/ still have `: any`, fix them before proceeding. The goal is to have minimal (<10) type errors remaining when we enable the flag.

Document findings and fix any remaining issues.
  </action>
  <verify>
```bash
cd apps/api && grep -c ": any" src/routes/*.ts src/services/*.ts src/middleware/*.ts 2>/dev/null | grep -v ":0$" | wc -l
```
Result should be 0 (no files with `: any`).
  </verify>
  <done>All route, service, and middleware files have 0 `: any` type annotations</done>
</task>
