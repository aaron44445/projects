...
export function PaymentForm({ onSuccess, onError, primaryColor = '#7C9A82' }: PaymentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      // CRITICAL: Pass paymentIntent.id (format: 'pi_xxx'), NOT the client secret
      // The booking endpoint needs the payment intent ID to link the appointment to the payment
