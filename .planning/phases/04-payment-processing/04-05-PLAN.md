...
**ID Validation Error Handling Test:**
5. Test handlePaymentSuccess with malformed ID to verify error handling:
   - In browser console, manually call: `handlePaymentSuccess('pi_xxx_secret_yyy')` (simulated malformed input)
   - Or temporarily modify PaymentForm to pass clientSecret instead of paymentIntent.id
   - **Expected result:** Error message displays "Payment processing error. Please try again."
   - **Expected result:** Booking is NOT created (no new appointment in database)
   - **Expected result:** Console shows error: `[handlePaymentSuccess] INVALID: Expected payment intent ID (pi_xxx), got: pi_xxx_secret_yyy`
   - If error handling works, restore PaymentForm to correct behavior
  </verify>
  <done>
...
8. **Explicit timing verification (PASS/FAIL criteria):**
   - Note timestamp T1 when payment confirmation appears in browser
   - Check Stripe CLI logs for webhook delivery timestamp T2
   - Query appointment: `SELECT depositStatus, updatedAt FROM Appointment WHERE stripePaymentIntentId = 'pi_xxx'`
   - Note appointment update timestamp T3
   - **PASS criteria:** (T3 - T1) < 60 seconds
   - **FAIL criteria:** (T3 - T1) >= 60 seconds or depositStatus not updated
   - Document: "Webhook timing: payment confirmed at [T1], depositStatus updated at [T3], delta = [T3-T1] seconds"
  </how-to-verify>
