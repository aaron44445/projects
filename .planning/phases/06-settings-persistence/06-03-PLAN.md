...
must_haves:
  truths:
    ...
    - "Selected location remains unchanged after page refresh (no auto-switch to primary)"
  artifacts:
...
**Step 3: Update fetchLocations auto-selection to respect localStorage (modify lines 131-137)**

Find the auto-selection block in fetchLocations (around lines 131-137, look for comment "Auto-select primary location if none selected"). Wrap the auto-selection logic with a localStorage check:
...
- Do not modify how locations are fetched from the API
  ...
  <verify>
    ...
    - Auto-selection respects localStorage: `grep -A 12 "Auto-select primary location" apps/web/src/hooks/useLocations.tsx`
  </verify>
