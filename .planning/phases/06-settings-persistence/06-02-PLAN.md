...
must_haves:
  truths:
    ...
    - "Service pricing changes via API reflect in booking widget"
...
Scope note: This phase stabilizes EXISTING settings infrastructure. Service pricing UI does not exist (only the API endpoint PUT /locations/:id/services/:serviceId). SET-03 will be tested via API, not UI. Building a service pricing UI is out of scope for this stabilization phase.
</objective>
...
- Service pricing API exists: PUT /locations/:id/services/:serviceId with priceOverride field
...
<task type="auto">
  <name>Task 1: Verify service pricing API exists and works</name>
  <files></files>
  <action>
Verify the service pricing API endpoint exists and the data flows correctly:

1. Confirm the PUT endpoint for service pricing exists:
   ```bash
   grep -A 20 "PUT.*:id/services/:serviceId\|put.*:id/services/:serviceId" apps/api/src/routes/locations.ts
   ```
   Should show endpoint accepting `priceOverride` in request body

2. Confirm price overrides are returned by public services endpoint:
   ```bash
   grep -B 5 -A 15 "priceOverride" apps/api/src/routes/public.ts
   ```
   Should show `effectivePrice: override.priceOverride ?? service.price` pattern

3. Document that service pricing UI does NOT exist (per phase scope):
   ```bash
   grep -r "priceOverride" apps/web/src/app/settings/ | grep -v ".map" | head -5
   ```
   Should return no UI components for editing price overrides

This task documents the scope boundary: API exists and works, UI doesn't exist, testing will use API directly.
  </action>
  <verify>
    - API endpoint exists: `grep "services/:serviceId" apps/api/src/routes/locations.ts`
    - Public endpoint uses overrides: `grep "effectivePrice" apps/api/src/routes/public.ts`
  </verify>
  <done>Confirmed service pricing API exists; UI does not exist (out of scope for stabilization); SET-03 will be tested via API</done>
</task>

<task type="auto">
  <name>Task 2: Verify availability service uses LocationHours</name>
  <files></files>
  <action>
...
<task type="auto">
  <name>Task 3: Verify public services endpoint serves fresh data</name>
  <files></files>
  <action>
...
2. Availability service fetches LocationHours from database (verified in Task 2)
3. Public services endpoint serves fresh pricing data (verified in Task 3)
4. Service pricing API exists (verified in Task 1) - no UI exists, testing via API
...
This test uses the API directly (no UI exists for location-specific service pricing):

1. Get a location ID and service ID from the database or API
2. Note the current price in the booking widget for that service at that location
3. Update the price override via API:
   ```bash
   curl -X PUT "$API_URL/api/v1/locations/YOUR_LOCATION_ID/services/YOUR_SERVICE_ID" \
     -H "Authorization: Bearer $TOKEN" \
     -H "Content-Type: application/json" \
     -d '{"priceOverride": 65}'
   ```
4. Refresh the booking widget
5. Verify the price shown is $65 (the override), not the original price

Alternative: Use the browser Network tab to call the API from the authenticated dashboard context.
  </how-to-verify>
  <resume-signal>
Report results for each test:
- SET-01 (settings persist): PASS/FAIL + details
- SET-02 (hours affect availability): PASS/FAIL + details
- SET-03 (pricing via API reflects in widget): PASS/FAIL + details
...
1. Service pricing API exists (Task 1)
2. Availability service queries LocationHours table (no hardcoded hours)
3. Public services endpoint applies price overrides from database
4. No application-level caching that would prevent fresh data
</verification>

<success_criteria>
- [ ] Service pricing API confirmed to exist; UI confirmed to NOT exist (scope boundary documented)
...
- [ ] Human verification: SET-03 (pricing via API reflects in widget) - PASS
</success_criteria>
