...
must_haves:
  truths:
    - "Booking creation is wrapped in database transaction with RepeatableRead isolation"
    - "Concurrent booking attempts for same slot result in exactly one success and others get TIME_CONFLICT error"
    - "P2034 serialization errors are automatically retried up to 5 times with exponential backoff"
    - "Row-level locking prevents race conditions during conflict check"
  artifacts:
...
4. Remove the duplicate conflict check code (lines 894-980) since the service now handles this atomically.
