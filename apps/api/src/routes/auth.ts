...
router.post('/login', loginRateLimit, asyncHandler(async (req: Request, res: Response) => {
  try {
    ...
    if (!user || !user.passwordHash) {
      // Track failed login attempt if user exists but has no password
      if (user) {
        await trackLoginAttempt(user.id, false, req, 'No password set');
      }
    ...
    if (!isValid) {
      ...
      await trackLoginAttempt(user.id, false, req, 'Invalid password');
      return res.status(401).json({
        success: false,
        error: {
          code: 'INVALID_CREDENTIALS',
    ...
    await trackLoginAttempt(user.id, true, req);
