...
locationsRouter.post('/', authorize('admin', 'owner'), asyncHandler(async (req, res, next) => {
  try {
    const { salonId } = req.user!;
    const { name, address, city, state, zip, country, phone, timezone, isPrimary } = req.body;
 ...
 * Get all services with location-specific effective pricing
 */
locationsRouter.get('/:id/services', asyncHandler(async (req, res, next) => {
  try {
    ...
    const overridesMap = new Map(
      serviceLocations.map(sl => [sl.serviceId, sl])
    );

    // Combine services with their location-specific settings
    const services = allServices.map((service) => {
      ...
      return {
        ...
        effectivePrice: override && override.priceOverride !== null ? Number(override.priceOverride) : service.price,
        effectiveDuration: override && override.durationOverride !== null ? Number(override.durationOverride) : service.durationMinutes,
        isEnabled: override ? override.isEnabled : true,
        hasOverride: override ? !!(override.priceOverride || override.durationOverride || override.isEnabled === false) : false,
      };
    });
