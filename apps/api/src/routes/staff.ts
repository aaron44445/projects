...
// Update staff member (self-edit or admin/owner for others)
// ============================================
router.patch('/:id', authenticate, asyncHandler(async (req: Request, res: Response) => {
  ...
  // Only admins/owners can change active status
  if (isActive !== undefined && !isAdminOrOwner) {
    return res.status(403).json({
      success: false,
      error: {
        code: 'FORBIDDEN',
        message: 'Only admins can change active status',
      },
    });
  }

  // Only admins/owners can change commission rate
  if (commissionRate !== undefined && !isAdminOrOwner) {
    return res.status(403).json({
      success: false,
      error: {
        code: 'FORBIDDEN',
        message: 'Only admins can change commission rate',
      },
    });
  }
...
// Set staff availability schedule (self-edit or admin/owner)
// ============================================
...
// Set staff services (which services they can perform) (admin/owner only)
// ============================================
router.put(
  '/:id/services',
  authenticate,
