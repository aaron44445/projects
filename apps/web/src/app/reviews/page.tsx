...
function ReviewsContent() {
  ...
  const renderStars = (rating: number) => {
    return Array.from({ length: 5 }).map((_, i) => (
      <Star
        key={i}
        className={`w-4 h-4 ${i < rating ? 'text-amber-400 fill-amber-400' : 'text-charcoal/20 dark:text-white/20'}`}
      />
    ));
  };
    ...
    <div className="min-h-screen bg-cream dark:bg-charcoal flex">
      ...
      <main className="flex-1 flex flex-col min-w-0">
        <header className="bg-white dark:bg-sidebar border-b border-charcoal/10 dark:border-white/10 px-6 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-2xl font-bold text-charcoal dark:text-white">Reviews</h1>
            </div>
        ...
        <div className="flex-1 p-6 overflow-auto">
          <FeatureGate feature="reviews">
            ...
            {/* Error State */}
            {error && (
              <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 rounded-xl border border-red-200 dark:border-red-500/30 flex items-center gap-3">
                <AlertCircle className="w-5 h-5 text-red-500" />
                <p className="text-sm text-red-700 dark:text-red-400">{error}</p>
                <button
                  onClick={() => fetchReviews()}
                  className="ml-auto px-4 py-2 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded-lg text-sm font-medium hover:bg-red-200 dark:hover:bg-red-900/50"
                >
                  Retry
                </button>
              </div>
            ...
            <div className="bg-gradient-to-br from-sage/10 to-lavender/10 dark:from-sage/20 dark:to-lavender/20 rounded-2xl p-6 border border-sage/20 dark:border-sage/30 mb-6">
              <h3 className="font-semibold text-charcoal dark:text-white mb-2">Review Tips</h3>
            ...
            <div className="bg-white dark:bg-sidebar rounded-2xl border border-charcoal/5 dark:border-white/10 shadow-soft">
              <div className="p-6 border-b border-charcoal/10 dark:border-white/10 flex items-center justify-between flex-wrap gap-4">
                <div>
                  <h2 className="text-lg font-semibold text-charcoal dark:text-white">Recent Reviews</h2>
                  <p className="text-sm text-charcoal/60 dark:text-white/60">Manage and respond to client feedback</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => setFilter('all')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === 'all' ? 'bg-sage/10 text-sage' : 'text-charcoal/60 dark:text-white/60 hover:bg-charcoal/5 dark:hover:bg-white/5'}`}
                  >
                    All ({pagination.total})
                  </button>
                  <button
                    onClick={() => setFilter('pending')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === 'pending' ? 'bg-sage/10 text-sage' : 'text-charcoal/60 dark:text-white/60 hover:bg-charcoal/5 dark:hover:bg-white/5'}`}
                  >
                    Pending ({pendingCount})
                  </button>
                  <button
                    onClick={() => setFilter('approved')}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === 'approved' ? 'bg-sage/10 text-sage' : 'text-charcoal/60 dark:text-white/60 hover:bg-charcoal/5 dark:hover:bg-white/5'}`}
                  >
              ...
              {loading ? (
                <div className="p-12 flex flex-col items-center justify-center">
                  ...
                  <p className="text-charcoal/60 dark:text-white/60">Loading reviews...</p>
                </div>
              ) : (
                <div className="divide-y divide-charcoal/10 dark:divide-white/10">
                  {filteredReviews.length === 0 ? (
                    <div className="p-12 text-center">
                      <Star className="w-12 h-12 text-charcoal/20 dark:text-white/20 mx-auto mb-4" />
                      <p className="text-charcoal/60 dark:text-white/60">No reviews found</p>
                    </div>
                  ) : (
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2 flex-wrap">
                              <p className="font-semibold text-charcoal dark:text-white">{getClientName(review.client)}</p>
                              <div className="flex items-center gap-0.5">{renderStars(review.rating)}</div>
                              <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                review.isApproved ? 'bg-sage/20 text-sage-dark' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'
                              }`}>
                                {review.isApproved ? 'approved' : 'pending'}
                              </span>
                            </div>
                            <p className="text-charcoal dark:text-white mb-2">{review.comment || 'No comment provided'}</p>
                            <div className="flex items-center gap-4 text-sm text-charcoal/50 dark:text-white/50">
                            ...
                            {review.responses.length > 0 && (
                              <div className="mt-4 p-4 bg-sage/5 dark:bg-sage/10 rounded-xl border-l-2 border-sage">
                                <p className="text-sm text-charcoal/60 dark:text-white/60 mb-1 font-medium">Your response:</p>
                                <p className="text-sm text-charcoal dark:text-white">{review.responses[0].responseText}</p>
              ...
              {pagination.totalPages > 1 && (
                <div className="p-4 border-t border-charcoal/10 dark:border-white/10 flex justify-center gap-2">
      ...
      {replyModal && (
        <div className="fixed inset-0 bg-charcoal/50 dark:bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-sidebar rounded-2xl w-full max-w-lg shadow-2xl">
            <div className="p-6 border-b border-charcoal/10 dark:border-white/10 flex items-center justify-between">
              <h2 className="text-xl font-semibold text-charcoal dark:text-white">Reply to Review</h2>
              <button onClick={() => { setReplyModal(null); setReplyText(''); }} className="p-2 hover:bg-charcoal/5 dark:hover:bg-white/5 rounded-lg">
            ...
            <div className="p-6">
              {/* Original Review */}
              <div className="bg-cream/50 dark:bg-charcoal/50 rounded-xl p-4 mb-6">
                <div className="flex items-center gap-2 mb-2">
                  <span className="font-medium text-charcoal dark:text-white">{getClientName(replyModal.client)}</span>
                  <div className="flex items-center gap-0.5">{renderStars(replyModal.rating)}</div>
                </div>
                <p className="text-sm text-charcoal/70 dark:text-white/70">{replyModal.comment || 'No comment provided'}</p>
              </div>

              {/* Reply Input */}
              <div>
                <label className="block text-sm font-medium text-charcoal dark:text-white mb-2">Your Response</label>
                <textarea
                  value={replyText}
                  onChange={(e) => setReplyText(e.target.value)}
                  placeholder="Thank you for your feedback..."
                  rows={4}
                  className="w-full px-4 py-3 border border-charcoal/20 dark:border-white/20 rounded-xl focus:outline-none focus:ring-2 focus:ring-sage/30 focus:border-sage bg-white dark:bg-charcoal dark:text-white dark:placeholder:text-white/40 resize-none"
                />
                <p className="text-xs text-charcoal/50 dark:text-white/50 mt-2">Your response will be visible on your booking page once the review is approved.</p>
              </div>
            </div>
